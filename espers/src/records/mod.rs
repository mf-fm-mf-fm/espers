...
pub use imad::{ImageSpaceAdapter, IMAD};
...
pub enum RawRecord {
    ...
    FSTP(FSTP),
    FSTS(FSTS),
    FURN(#[br(args(localized))] FURN),
    GLOB(GLOB),
    GMST(#[br(args(localized))] GMST),
    GRAS(GRAS),
    GRUP(#[br(args(localized))] GRUP),
    HAZD(#[br(args(localized))] HAZD),
    HDPT(#[br(args(localized))] HDPT),
    IDLE(IDLE),
    IDLM(IDLM),
    IMAD(IMAD),
    IMGS(IMGS),
    INFO(INFO),
    INGR(#[br(args(localized))] INGR),
    IPCT(IPCT),
    IPDS(IPDS),
    KEYM(#[br(args(localized))] KEYM),
    KYWD(KYWD),
    LAND(LAND),
    LCRT(LCRT),
    LCTN(LCTN),
...
pub enum Record {
    ...
    ImageSpaceAdapter(ImageSpaceAdapter),
...
impl TryFrom<RawRecord> for Record {
    ...
    fn try_from(r: RawRecord) -> Result<Self, Self::Error> {
        match r {
            ...
            RawRecord::IMAD(x) => Ok(Record::ImageSpaceAdapter(ImageSpaceAdapter::try_from(x)?)),
...
impl fmt::Display for Record {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Record::ImageSpaceAdapter(x) => write!(f, "{}", x),
...
impl Record {
    pub fn magic(&self) -> [u8; 4] {
        match self {
            ...
            Record::ImageSpaceAdapter(_) => *b"IMAD",
    ...
    pub fn form_id(&self) -> Option<u32> {
        match self {
            ...
            Record::ImageSpaceAdapter(rec) => Some(rec.header.form_id),
